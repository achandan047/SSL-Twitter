...
def get_weights(dataset):
    ...
    minlabel, maxlabel = min(labels), max(labels)
    n_bins = 7
    step = (maxlabel - minlabel) / n_bins
    
    cnt_bins = [0 for _ in range(n_bins)]
    bin_map = [-1 for _ in range(n_labels)]
    for i, label in enumerate(labels):
        binnum = int((label - minlabel) / step)
        if binnum == n_bins: binnum -= 1
        bin_map[i] = binnum
        cnt_bins[binnum] += 1
...
#     plt.show()
    ...
    weights_per_bin = [min(10.,(len(labels) / cnt_bins[b])) if cnt_bins[b] != 0 else 0 for b in range(n_bins)]
    weights = [weights_per_bin[bin_map[i]] for i in range(n_labels)]
...
def add_ensemble_data(values, variances, config, label="Label", iteration=0):
    ...
    train_df.append(certain_df, sort=True)
    ...
    unlabeled_df.append(uncertain_df, sort=True)
